...
class PengobatanAjax
{
    ...
    public function handleRequest()
    {
        ...
        switch ($action) {
            ...
            case 'getPatientReadings':
                ...
                break;
            case 'getMedications':
                echo json_encode($this->getMedications());
                break;
        }
    }
    ...
    private function getPatientReadings($patientId)
    {
        $query = "SELECT bpr.reading_id, bpr.systolic, bpr.diastolic, 
                         bpr.pulse_rate, bpr.reading_date
    ...
    {
        $search = isset($_POST['search']) ? $_POST['search'] : '';
        $query = "SELECT m.medication_id, m.name, m.dosage_form, m.description 
                 FROM medications m 
                 WHERE m.name LIKE ? 
                 ORDER BY m.name ASC";
        return $this->db->query($query, ["%$search%"])->fetchAll();
